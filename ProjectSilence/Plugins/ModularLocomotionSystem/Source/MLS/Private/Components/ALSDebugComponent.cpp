                                       ...
                                       FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (bShowDebugShapes)
	{
		...
		if (Controller)
		{
			AMLSPlayerCameraManager* CamManager = Cast<AMLSPlayerCameraManager>(Controller->PlayerCameraManager);
			if (CamManager)
			{
...
void UALSDebugComponent::FocusedDebugCharacterCycle(bool bValue)
{
	...
	{
		// Return here as no AMLSBaseCharacter where found during call of BeginPlay.
...
void UALSDebugComponent::BeginPlay()
{
	...
	OwnerCharacter = Cast<AMLSBaseCharacter>(GetOwner());
...
{
	// Get all MLSBaseCharacter's, which are currently present to show them later in the ALS HUD for debugging purposes.
	TArray<AActor*> AlsBaseCharacters;
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), AMLSBaseCharacter::StaticClass(), AlsBaseCharacters);
	...
	if (AlsBaseCharacters.Num() > 0)
	{
		...
		{
			if (AMLSBaseCharacter* AlsBaseCharacter = Cast<AMLSBaseCharacter>(Character))
			{
		...
		{ // seems to be that this component was not attached to and AMLSBaseCharacter,
...
void UALSDebugComponent::ToggleDebugView()
{
	...
	AMLSPlayerCameraManager* CamManager = Cast<AMLSPlayerCameraManager>(
		UGameplayStatics::GetPlayerCameraManager(GetWorld(), 0));
	if (CamManager)
	{
		UMLSPlayerCameraBehavior* CameraBehavior = Cast<UMLSPlayerCameraBehavior>(
